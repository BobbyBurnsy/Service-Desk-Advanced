...
if (-not (Get-MgContext -ErrorAction SilentlyContinue)) {
    if ($NoPrompt) {
        # Fail silently for background auto-fetches so we don't block the API Gateway
        Write-Output '{"error":"Graph API not connected. Skipping background fetch."}'
        return
    }
...
try {
    switch ($Action) {
        ...
        "AddSMS" {
            ...
            Write-Output "<div style='color:#2ecc71;'><i class='fa-solid fa-check'></i> [SDA SUCCESS] $PhoneNumber added as primary SMS MFA.</div>"
        }
    }
} catch {
