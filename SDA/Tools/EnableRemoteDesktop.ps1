<#
.SYNOPSIS
    SDA Web-Ready Tool: EnableRemoteDesktop.ps1
.DESCRIPTION
...
if ($GetTrainingData) {
    $data = @{
        StepName = "ENABLE REMOTE DESKTOP"
        Description = "Enabling RDP manually requires modifying three separate security layers: the Registry, the Firewall, and the Terminal Service. Typing all of that out in a console is a nightmare. This is the perfect time to learn the Golden Rule of IT: Don't memorize long commands. Build a script library. By utilizing Sysinternals PsExec, you can remotely invoke PowerShell to execute a pre-written script directly from your IT network share as the SYSTEM account."
        Code = "psexec \\`$Target -s powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File `"\\server\IT_Share\Scripts\EnableRDP.ps1`""
...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] ENABLE REMOTE DESKTOP"
Write-Output "========================================"
...
try {
    ...
    Write-Output "`n[SDA SUCCESS] Remote Access provisioned successfully via WinRM!"
    ...
    if (Test-Path $psExecPath) {
        try {
            ...
            if ($Process.ExitCode -eq 0) {
                Write-Output "`n[SDA SUCCESS] Remote Access provisioned successfully via PsExec!"
                Write-Output "[i] You can try connecting using MSRA or RDP now."
