<#
.SYNOPSIS
    SDA Web-Ready Tool: InstalledSoftwareAudit.ps1
.DESCRIPTION
    Remotely queries the target computer's registry to compile a list of installed software.
    Uses Base64 transit encoding to prevent stream corruption from trademark/copyright symbols.
...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] INSTALLED SOFTWARE AUDIT"
Write-Output "========================================"
...
$ExportPath = "$LocalTemp\SoftwareAudit_$Target_$Timestamp.csv"
...
if ($RawOutputString -match '---B64_START---(.*?)---B64_END---') {
    try {
        ...
        $appData = $jsonString | ConvertFrom-Json
        if ($appData -isnot [System.Array]) { $appData = @($appData) }

        if ($appData.Count -gt 0) {
            Write-Output "`n[SDA SUCCESS] Found $($appData.Count) installed applications via $MethodUsed."

            # Export to CSV
            $appData | Export-Csv -Path $ExportPath -NoTypeInformation -Force
        ...
        Write-Output "`n[!] ERROR: Failed to decode Base64 software data."
    }
} else {
