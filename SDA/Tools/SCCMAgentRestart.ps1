...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] SCCM AGENT RESTART"
Write-Output "========================================"
...
try {
    ...
    Write-Output "`n[SDA SUCCESS] SCCM Agent restarted successfully via WinRM!"
    ...
    if (Test-Path $psExecPath) {
        try {
            ...
            if ($Process.ExitCode -eq 0) {
                Write-Output "`n[SDA SUCCESS] SCCM Agent restarted successfully via PsExec!"
                Write-Output "[i] Note: It may take 2-3 minutes for the endpoint to check in with the Management Point."
