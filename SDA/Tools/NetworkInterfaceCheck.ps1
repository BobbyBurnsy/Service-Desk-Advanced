...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] NETWORK INTERFACE DIAGNOSTICS"
Write-Output "========================================"
...
$ActionLog = "Network Interface Diagnostics Executed"
...
if ($RawOutputString -match '---JSON_START---(.*?)---JSON_END---') {
    try {
        ...
        if ($netData.Count -gt 0) {
            Write-Output "`n[SDA SUCCESS] Network interfaces retrieved via $MethodUsed!`n"
            ...
            foreach ($nic in $netData) {
                ...
                $html += "<span style='color: #94a3b8;'>Description:</span><span style='color: #cbd5e1;'>$($nic.Desc)</span>"
                $html += "<span style='color: #94a3b8;'>IPv4 Address:</span><span style='color: #2ecc71; font-weight: bold;'>$($nic.IP)</span>"
                $html += "<span style='color: #94a3b8;'>MAC Address:</span><span style='color: #cbd5e1; font-family: Consolas, monospace;'>$($nic.MAC)</span>"
                $html += "<span style='color: #94a3b8;'>Link Speed:</span><span style='color: #cbd5e1;'>$($nic.Speed)</span>"
                $html += "</div></div>"
            }
