...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] AUTOMATED WARRANTY ROUTING"
Write-Output "========================================"
...
if ($RawOutputString -match '---JSON_START---(.*?)---JSON_END---') {
    try {
        ...
        if ($hwData.Count -gt 0) {
            Write-Output "`n[SDA SUCCESS] Hardware telemetry retrieved via $MethodUsed!`n"
