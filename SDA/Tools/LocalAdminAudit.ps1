...
# --- Main Execution ---
Write-Output "========================================"
Write-Output "[SDA] LOCAL PRIVILEGE AUDITING"
Write-Output "========================================"
...
if ($RawOutputString -match '---JSON_START---(.*?)---JSON_END---') {
    try {
        ...
        if ($adminData.Count -gt 0) {
            Write-Output "`n[SDA SUCCESS] Local Administrators retrieved via $MethodUsed!`n"
