...
if (-Not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # If running from a mapped drive, resolve to true UNC path so the elevated session can find it
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host " Docs: www.servicedeskadvanced.com" -ForegroundColor DarkGray
    ...
    while ($HttpListener.IsListening) {
        try {
            ...
            elseif ($Request.Url.AbsolutePath -eq "/api/remote/connect" -and $Request.HttpMethod -eq "POST") {
                # Enforce UTF-8 Encoding on incoming streams
            ...
            # --- AD Identity Actions (PURE LOCAL AD) ---
            elseif ($Request.Url.AbsolutePath -eq "/api/identity/action" -and $Request.HttpMethod -eq "POST") {
                ...
                # Safely trim the password to prevent accidental copy/paste spaces
                ...
                try {
                    ...
                    elseif ($Action -eq "ResetPassword") { 
                        ...
                        Set-ADUser -Identity $TargetUser -ChangePasswordAtLogon $true -ErrorAction Stop

                        $ResponseObj.message = "Password Reset Successfully: $NewPassword" 
                    }
                } catch {
                    # Capture the EXACT Active Directory error message to pass back to the UI
                    $errMsg = $_.Exception.Message
                    Write-Host "  > [!] AD Action Failed: $errMsg" -ForegroundColor Red
                    $ResponseObj.status = "error"
                    $ResponseObj.message = "AD Error: $errMsg"
                }
            ...
            # THE RESILIENT LOOP: If any command throws a terminating error, catch it here so the server stays alive.
